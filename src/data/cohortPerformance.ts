...
function makePacing(totalEnrolls: number, durationDays: number, complete = true): DailyPacing[] {
  const points: DailyPacing[] = [];
  let cumulative = 0;
  const activeDays = complete ? durationDays : Math.round(durationDays * 0.55);
...
const whartonSpring2026: CohortData = {
  ...
  pacing: makePacing(964, 120, false),
};
...
const columbiaWinter2026: CohortData = {
  ...
  pacing: makePacing(284, 90, false),
};
